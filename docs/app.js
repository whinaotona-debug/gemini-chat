...
const KEY_STORAGE = "gemini-browser-api-key";
...
const CHAT_MODELS = ["gemini-2.5-flash", "gemini-2.0-flash", "gemini-flash-latest", "gemini-1.5-flash"];
const IMAGE_MODELS = [
...
let browserKey = localStorage.getItem(KEY_STORAGE) || "";
let chatModel = CHAT_MODELS[0];
let imageModel = IMAGE_MODELS[0];
...
function partsToPayload(parts) {
  ...
  for (const part of parts) {
    ...
    images.push({ mimeType, dataUrl: `data:${mimeType};base64,${inline.data}` });
  }
  return { text, images };
}
...
async function readSseChat(res, modelEl) {
  ...
  while (true) {
    ...
    for (const chunk of chunks) {
      ...
      if (event.type === "text") {
        ...
        const { text, images } = partsToPayload(collectParts(event));
        if (text) {
          modelEl.text = (modelEl.text || "") + text;
          modelEl.pending = null;
          renderThread();
        }
        if (images.length) {
          modelEl.images = [...(modelEl.images || []), ...images];
          renderThread();
        }
        if (event.error) throw new Error(sanitizeError(event.error.message || "エラー"));
      }
    }
  }
}
...
async function streamChat(convo, modelEl) {
  if (useLocalApi) {
    ...
    await readSseChat(res, modelEl);
    return;
  }
  if (!browserKey) throw new Error("左の API キーを保存してください。");
  let lastError = "チャットに失敗しました。";
  for (const model of [chatModel, ...CHAT_MODELS.filter((m) => m !== chatModel)]) {
    const res = await fetch(`${GEMINI_BASE}/models/${model}:streamGenerateContent?alt=sse`, {
      ...
      body: JSON.stringify({
        contents: toGeminiContents(apiMessages(convo)),
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      lastError = sanitizeError(data.error?.message || `Gemini エラー (${res.status})`);
...
async function generateImage(convo, modelEl, prompt) {
  ...
  if (!browserKey) throw new Error("左の API キーを保存してください。");
  const contents = toGeminiContents(apiMessages(convo));
  let lastError = "画像生成に失敗しました。";
  for (const model of [imageModel, ...IMAGE_MODELS.filter((m) => m !== imageModel)]) {
    const payloads = [
      {
        contents,
        generationConfig: {
    ...
    for (const body of payloads) {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        lastError = sanitizeError(data.error?.message || `画像生成エラー (${res.status})`);
        if (/imageConfig|unknown name/i.test(lastError)) continue;
        if (res.status === 404) break;
        throw new Error(lastError);
      }
      imageModel = model;
      const parsed = partsToPayload(collectParts(data));
      if (!parsed.images.length) {
        lastError = parsed.text || "画像が返りませんでした。";
        continue;
      }
      modelEl.text = parsed.text || "";
      modelEl.images = parsed.images;
      modelEl.pending = null;
      return;
    }
  }
...
async function refreshHealth() {
  ...
  if (!browserKey) {
    status.className = "key-status bad";
    status.textContent = "サイト用に API キーを保存してください。";
    return;
  }
  try {
    const res = await fetch(`${GEMINI_BASE}/models?pageSize=20`, {
      headers: { "x-goog-api-key": browserKey },
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      status.className = "key-status bad";
      status.textContent = sanitizeError(data.error?.message || "API キーを確認してください。");
      return;
    }
    status.className = "key-status ok";
    status.textContent = `接続OK（${chatModel}）`;
  } catch {
    status.className = "key-status bad";
    status.textContent = "Gemini に接続できません。";
  }
}
